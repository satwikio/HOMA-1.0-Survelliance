...
from go_align_drop import go_align_drop, DroneController
...
from siyi_cam2 import SIYICam  # Import your class
...
class DroneClient:
    ...
    async def start_webrtc(self):
        ...
        if self.drone_type == "recon":
            ...
            self.video_track = WebcamVideoTrack(detection_callback=None)
    ...
    async def process_fire_queue(self):
        ...
        try:
            ...
            # Start position monitor for altitude tracking
            asyncio.create_task(drone_controller._monitor_position())

            print(f"🚁 Executing go_align_drop for {len(gps_list)} fire locations...")

            # Execute the proven go_align_drop function
