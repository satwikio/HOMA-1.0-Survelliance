...
try:
    ...
    SIYICam = None
    print("⚠️ Warning: siyi_cam2 module not found. Camera features will be disabled.")
...
ALIGN_CHECK_HZ = 10.0
ALIGN_TIMEOUT = 45.0  # seconds to try aligning before giving up and continuing
...
ALTITUDE = 15.0                 # default altitude to travel to (m) if per-point altitude is not provided
...
class DroneController:
    def __init__(self, connection_string="udpin://0.0.0.0:14540"):
        ...
        self.mission_cancelled = False  # Cancel mission and hold position
        self.rtl_requested = False      # Return to launch

        # last known telemetry (updated by monitor or shared from parent)
        self.altitude = None  # in meters (relative altitude when available)
        self.last_lat = None  # Last known latitude
        self.last_lon = None  # Last known longitude
    ...
    async def _monitor_position(self):
        """Continuously read telemetry.position() to keep latest position cached."""
        try:
            async for pos in self.drone.telemetry.position():
                ...
                # Also cache lat/lon for distance calculations
                try:
                    self.last_lat = float(pos.latitude_deg)
                    self.last_lon = float(pos.longitude_deg)
                except Exception:
                    pass

        except Exception as e:
            print(f"❌ Position monitor STOPPED: {type(e).__name__}: {e}")
            print(f"   Telemetry stream is broken - distance calculations will fail!")
            self.altitude = None
            self.last_lat = None
            self.last_lon = None
    ...
    def cancel_mission(self):
        """Cancel current mission and switch to position hold mode"""
        self.mission_cancelled = True
        print("🛑 Mission cancel requested - will hold position")

    def request_rtl(self):
        """Request return to launch - cancels mission and executes RTL"""
        self.rtl_requested = True
        print("🏠 RTL requested - will return to launch")
    ...
    async def goto_location_mission(self, lat, lon, alt, speed_m_s: float = 5.0, wait_for_arrival: bool = False):
        ...
        try:
            ...
            # NOTE: Takeoff already handled by caller (script.py), skip here
    ...
    # Helper to check distance to a GPS target (uses cached position)
    async def distance_to_target(self, target_lat, target_lon):
        """
        Returns approximate horizontal distance (meters) to target using cached position.
        Uses cached lat/lon to avoid creating duplicate telemetry subscriptions.
        """
        try:
            # Use cached position data (updated by _monitor_position or shared from parent)
            if self.last_lat is None or self.last_lon is None:
                print(f"⚠️ No cached position data available yet")
                # Wait briefly for position data to be available
                await asyncio.sleep(0.2)
                if self.last_lat is None or self.last_lon is None:
                    print(f"❌ Still no position data after waiting")
                    return float('inf')

            dist = haversine_meters(self.last_lat, self.last_lon, target_lat, target_lon)
            return dist

        except Exception as e:
            print(f"❌ Error calculating distance: {type(e).__name__}: {e}")
            import traceback
            traceback.print_exc()
            return float('inf')
...
# -----------------------------
async def go_align_drop(drone: DroneController, cam: Optional[Any], gps_list, approach_alt=ALTITUDE):
    """
    ...
    if cam is not None:
        try:
            ...
            raise RuntimeError(f"Camera setup failed - cannot proceed with fire extinguishing: {e}")
    else:
        print("⚠️ Camera not available, ArUco detection disabled")
    ...
    for idx, g in enumerate(gps_list):
        ...
        try:
            ...
            if not ok:
                print("⚠️ goto_location_mission returned False - drone may not navigate!")
            else:
                navigation_ok = True
                print("✅ Navigation command sent successfully")
        except Exception as e:
            print(f"⚠️ goto_location_mission exception: {e}")

        # wait until within APPROACH_THRESHOLD_METERS or timeout
        approach_deadline = time() + 60.0
        inf_count = 0  # Track consecutive inf distance readings
        while True:
            # Check for RTL or cancel
            if drone.rtl_requested:
                print("\n🏠 RTL requested during approach - executing return to launch")
                try:
                    await drone.stop_offboard()
                    await drone.drone.action.return_to_launch()
                except Exception as e:
                    print(f"⚠️ RTL execution failed: {e}")
                return

            if drone.mission_cancelled:
                print("\n🛑 Mission cancelled during approach - holding position")
                try:
                    await drone.stop_offboard()
                    # Position hold is automatic when exiting offboard mode
                except Exception as e:
                    print(f"⚠️ Position hold failed: {e}")
                return

            dist = await drone.distance_to_target(lat, lon)

            # Fail-fast if distance is consistently inf (telemetry broken)
            if dist == float('inf'):
                inf_count += 1
                print(f"   Distance to target: inf m (telemetry broken, attempt {inf_count}/5)", end="\r")
                if inf_count >= 5:
                    print(f"\n❌ CRITICAL: Telemetry broken for {inf_count} consecutive attempts")
                    print(f"   Cannot navigate without position data - ABORTING waypoint")
                    raise RuntimeError("Telemetry stream broken - cannot get distance to target")
            else:
                inf_count = 0  # Reset counter on valid reading
                print(f"   Distance to target: {dist:.1f} m", end="\r")

            if dist <= APPROACH_THRESHOLD_METERS:
                print(f"\n✅ Reached approach radius ({APPROACH_THRESHOLD_METERS} m). Starting ArUco alignment.")
                break
            if time() > approach_deadline:
                if dist == float('inf'):
                    print("\n❌ CRITICAL: Timeout AND telemetry broken - cannot proceed")
                    raise RuntimeError("Timeout approaching target with broken telemetry")
                print(f"\n⚠️ Timeout while approaching (still {dist:.1f}m away) - proceeding to alignment attempt anyway.")
                break
            await asyncio.sleep(1.0)
        ...
        if not started:
            print("❌ CRITICAL: Offboard mode failed to start")
            print("   This usually means MAVSDK connection is broken")
            print("   Cannot perform precision alignment without offboard control")
            raise RuntimeError("Offboard mode failed - MAVSDK connection likely broken")

        print("✅ Offboard mode started - beginning precision alignment")
        aligned_count = 0
        start_time = time()
        last_frame = None
        ...
        try:
            ...
            while True:
                # Check for RTL or cancel
                if drone.rtl_requested:
                    print("🏠 RTL requested during alignment - executing return to launch")
                    await drone.stop_offboard()
                    try:
                        await drone.drone.action.return_to_launch()
                    except Exception as e:
                ...
                if drone.mission_cancelled:
                    print("🛑 Mission cancelled during alignment - holding position")
                    await drone.stop_offboard()
                    return

                # read frame from camera frames() generator if available, else get_frame()
                frame = None
                if cam is not None:
                    try:
                        frames_iter = getattr(cam, "frames", None)
                        if callable(frames_iter):
                            # attempt to retrieve one frame by calling generator and next()
                            gen = frames_iter()
                            frame = next(gen)
                        else:
                            frame = cam.get_frame()
                        ...
                        # frames() might expect to be iterated; fallback to get_frame
                        try:
                            frame = cam.get_frame()
                        except Exception:
                            frame = None
...
async def main():
    ...
    cam = SIYICam() if SIYICam is not None else None  # uses defaults; pass args if your constructor requires (ip, port, ...)
    if cam is None:
        print("⚠️ Camera not available, continuing without camera features")
