...
#                 takeoff_timeout = 30.0  # 30 second timeout

#                 while time.time() - takeoff_start < takeoff_timeout:
#                     if "gps" in self.latest and "alt_rel_m" in self.latest["gps"]:
#                         alt = self.latest["gps"]["alt_rel_m"]
#                         if alt >= 8.0:  # Close enough to 10m target
#                             print(f"✅ Takeoff complete at {alt:.1f}m")
#                             break
#                         if int(time.time() - takeoff_start) % 2 == 0:  # Log every 2 seconds
#                             print(f"⏳ Climbing... altitude: {alt:.1f}m")
#                     await asyncio.sleep(0.5)

#                 # Extra wait for stabilization
#                 await asyncio.sleep(2)
#                 self.is_airborne = True
#                 print("✅ Drone airborne and stabilized, ready for missions")
#             except Exception as e:
#                 print(f"❌ Takeoff failed: {e}")
#                 print(f"⚠️ Continuing anyway (may already be airborne)")
#                 self.is_airborne = True  # Assume airborne to proceed
...
#         # go_align_drop expects list of tuples: (lat, lon, alt)
#         gps_list = [
#             (fire["target_lat"], fire["target_lon"], 10.0)  # Use 15m altitude
#             for fire in fire_list
#         ]
...
#             # Create DroneController wrapper around our MAVSDK drone
#             # IMPORTANT: We reuse the existing connected drone to avoid breaking telemetry
#             drone_controller = DroneController(connection_string="", shared_mode=True)
#             drone_controller.drone = self.drone  # Use our already-connected drone
...
#         # Reset airborne status after landing so next mission can take off
#         self.is_airborne = False
#         print("🛬 Drone landed, ready for next mission")
