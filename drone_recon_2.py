...
# === Configuration ===
BACKEND_URL = "http://100.88.6.111:8000"
WS_URL = "ws://100.88.6.111:8000"
...
class DroneClient:
        ...
        try:
            ...
            if cmd == "arm":
                data = {
                    ...
                    "area": {
                        "nw": {"lat": 31.782797, "lng": 77.003261},
                        "se": {"lat": 31.782576, "lng": 77.004067}
                    },
                    "grid_spacing": 10,
                    "altitude": 5,
                    "speed": 5
    ...
    async def clear_existing_mission(self, timeout: float = 5.0, retries: int = 2):
        ...
        for attempt in range(1, retries + 1):
            try:
                ...
                # give the autopilot a short moment to apply the change
                await asyncio.sleep(0.5)

                # Optionally try to verify by downloading mission (if supported)
                # Some autopilots/versions might not return an empty mission reliably,
                # but this is a reasonable verification attempt:
                try:
                    mission_items = []
                    ...
                    # If download_mission isn't available or errors out, assume clear likely worked.
    ...
    async def handle_survey_mission(self, data):
        ...
        try:
            area = data.get("area", {})  # Expecting {'nw': {'lat':..., 'lng':...}, 'se': {...}}
            grid_spacing = data.get("grid_spacing", 10)  # in meters
            mission_id = data.get("mission_id", "unknown")
            altitude = data.get("altitude", 25)
            speed = data.get("speed", 5)
            ...
            planner = PreciseSurveyPlanner(
                polygon_coords=area,        # your polygon list [(lat,lon), ...]
                altitude_m=altitude,
            ...
            if not ok:
                # Decide policy: continue (upload will commonly overwrite), or abort
                print("Warning: could not clear previous mission; proceeding to upload (may overwrite).")
            # now upload new mission
            ...
            # Immediately return to caller so handle_command can finish and WS loop remains active
            return
