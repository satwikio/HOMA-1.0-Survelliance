...
class DroneController:
    def __init__(self, connection_string="udp://:14540", shared_mode=False):
        self.drone = System()
        self.connection_string = connection_string
        self.shared_mode = shared_mode  # If True, don't start telemetry monitors
        ...
        # last known telemetry (updated by monitor or externally if shared)
        self.altitude = None  # in meters (relative altitude when available)

    async def connect(self):
        if self.shared_mode:
            print("🔗 [SHARED MODE] Using externally connected drone, skipping connection and monitors")
            return
        ...
        # start monitors only if not in shared mode
        print("🔗 Starting telemetry monitors...")
...
async def go_align_drop(drone: DroneController, cam: SIYICam, gps_list, approach_alt=ALTITUDE):
    ...
    for idx, g in enumerate(gps_list):
        ...
        while time() < approach_deadline:
            try:
                # Use asyncio.wait_for to prevent blocking forever
                async def check_progress():
                    async for progress in drone.drone.mission.mission_progress():
                        return progress

                progress = await asyncio.wait_for(check_progress(), timeout=2.0)
                progress_check_count += 1
                elapsed = time() - approach_start_time

                if progress_check_count % 5 == 0:  # Log every 5 checks
                ...
                # Mission progress stream timed out, check if we should continue waiting
                elapsed = time() - approach_start_time
                if progress_check_count == 0:
                    print(f"⏳ [APPROACH] Waiting for mission to start... ({elapsed:.1f}s)")
                continue
            except Exception as e:
                print(f"⚠️  [APPROACH] Error reading mission progress: {e}")
                await asyncio.sleep(0.5)
                continue
