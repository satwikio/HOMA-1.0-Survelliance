...
class WebcamVideoTrack(VideoStreamTrack):
    ...
    async def recv(self):
        ...
        # Process frame for object detection (pass BGR frame)
...
class DroneClient:
    def __init__(self):
        ...
        if self.drone_type == "recon":
            ...
            self.frame_count = 0
            self.detection_interval = 10  # Run detection every 10 frames (~0.33s at 30fps)
        ...
        if self.drone_type == "rescue":
            ...
            self.current_mission = None  # Currently active extinguish mission
    ...
    async def start_webrtc(self):
        # ONLY for surveillance (recon) drones - payload drones don't need video
        if self.drone_type != "recon":
            print("📦 Payload drone - Skipping WebRTC video setup")
            return
        ...
        # Create fresh connections with detection callback
        self.video_track = WebcamVideoTrack(detection_callback=self.process_frame_for_detection)
    ...
    async def process_frame_for_detection(self, frame):
        ...
        # Only run detection every N frames to reduce load
    ...
    async def process_fire_queue(self):
        """Process fire queue one by one (FIFO) - FOR PAYLOAD DRONES ONLY"""
        if self.drone_type != "rescue":
            return

        print(f"\n🔥 Starting fire queue processor...")

        while self.fire_queue:
            # Get next fire from queue
            fire = self.fire_queue.pop(0)
            self.current_mission = fire
            ...
            print(f"\n{'='*60}")
            print(f"🚁 PROCESSING FIRE {len(self.fire_queue) + 1}")
            print(f"{'='*60}")
            print(f"Target: ({target_lat}, {target_lon})")
            print(f"Remaining in queue: {len(self.fire_queue)}")
            print(f"{'='*60}\n")
            ...
            try:
                await self.websocket.send(json.dumps({
                    ...
                    "mission_id": mission_id,
                    "queue_remaining": len(self.fire_queue)
                }))
            except Exception as e:
                print(f"Failed to send status update: {e}")

            # TODO: Implement actual navigation and fire extinguishing
            # For now, simulate mission execution
            print(f"📍 Navigating to ({target_lat}, {target_lon})..."ex
            await asyncio.sleep(3)  # Simulate travel time

            print(f"💧 Extinguishing fire...")
            await asyncio.sleep(2)  # Simulate extinguish time
        ...
        print(f"\n✅ Fire queue completed! All fires extinguished.\n")
    ...
    async def run(self):
        ...
        async with websockets.connect(ws_url) as ws:
            ...
            # 4. Setup WebRTC (only for surveillance drones)
            await self.start_webrtc()

            if self.drone_type == "recon" and self.pc:
                offer = await self.pc.createOffer()
                await self.pc.setLocalDescription(offer)
                msg = {"type": "webrtc_offer", "drone_id": DRONE_ID, "sdp": self.pc.localDescription.sdp}
