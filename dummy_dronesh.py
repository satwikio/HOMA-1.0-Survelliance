...
class DroneClient:
    ...
    async def handle_stop_mission(self, data):
        """Handle stop mission command - puts drone in HOLD mode"""
        print(f"\n{'='*60}")
        print(f"🛑 STOP MISSION COMMAND RECEIVED")
        print(f"{'='*60}")

        try:
            # Put drone in HOLD mode (loiter at current position)
            print(f"🔵 Setting flight mode to HOLD...")
            await self.drone.action.hold()

            print(f"✅ Drone is now in HOLD mode")

            # Send success response
            resp = {
                "type": "stop_mission_response",
                "drone_id": DRONE_ID,
                "status": "success",
                "message": "Drone is now in HOLD mode"
            }
            ...
            print(f"❌ Failed to enter HOLD mode: {e}")
            resp = {
                "type": "stop_mission_response",
                "drone_id": DRONE_ID,
                "status": "error",
        ...
        try:
            ...
            async with websockets.connect(ws_url) as ws:
                ...
                async for message in ws:
                    ...
                    elif typ == "stop_mission":
                        await self.handle_stop_mission(data)
                    else:
