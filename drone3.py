...
#                 print("✅ Drone armed")
#             except Exception as e:
#                 print(f"⚠️ Arm failed (maybe already armed): {e}")
...
#                 # Assume airborne to proceed if arming/takeoff errors are ignored
#                 self.is_airborne = True 

#         # Get all fires from queue
#         fire_list = self.fire_queue.copy()
#         print(f"📝 Processing {len(fire_list)} fires from queue")
...
#         # Prepare GPS list
#         gps_list = [
#             (fire["target_lat"], fire["target_lon"], 15.0)
#             for fire in fire_list
#         ]

#         # Execute batch fire extinguishing
#         try:
#             print(f"🚁 Executing go_align_drop for {len(gps_list)} fire locations...")
            
#             # --- CHANGE HERE: Use the shared camera instance ---
#             # Do NOT create 'cam = SIYICam()' here.
#             # Use self.cam (initialized in __init__) or the global siyi_cam_
            
#             # Note: go_align_drop will use this camera object to read frames.
#             # Ensure SIYICam.get_frame() works while WebRTC is running.
#             await go_align_drop(self.drone, self.cam, gps_list)

#             # Generate Results (Assumes success if no exception raised)
#             results = [
#                 {
#                     "success": True,
...
#         async with self.mission_lock:
#             for fire in fire_list:
#                 if fire in self.fire_queue:
#                     self.fire_queue.remove(fire)
#             self.current_mission = None
#             print(f"🧹 Queue processed. {len(self.fire_queue)} items remaining.")
...
# DRONE_ID = os.getenv("DRONE_ID", "austin-recon-01")
# SECRET_KEY = os.getenv("SECRET_KEY", "drone_aus_123")
# DRONE_TYPE = os.getenv("DRONE_TYPE", "recon")  # "recon" or "rescue"
# USE_DUMMY_TELEMETRY = False

# # === Global Camera Instance ===
# # We initialize it here but start the thread inside DroneClient
# siyi_cam_ = SIYICam()
...
#         if self.consecutive_send_failures >= self.max_consecutive_failures:
#             print("❌ NETWORK FAILURE DETECTED: Restarting...")
#             time.sleep(self.retry_delay)
#             sys.exit(EXIT_NETWORK_FAILURE)
...
#         if now - self.last_health_check > 10:
#             self.last_health_check = now
#             if self.frames_sent_count > 0 and (now - self.last_frame_sent > self.frame_timeout):
#                 print(f"⚠️ VIDEO STREAM STALLED ({now - self.last_frame_sent:.1f}s)")
...
#     def on_telemetry_send_failed(self, error=None):
#         self.consecutive_send_failures += 1
#         print(f"⚠️ Telemetry fail #{self.consecutive_send_failures}")
#         if self.consecutive_send_failures >= self.max_consecutive_failures:
#             print("❌ TELEMETRY FAILURE: Backend connection lost")
#             sys.exit(EXIT_BACKEND_UNAVAILABLE)


# class WebcamVideoTrack(VideoStreamTrack):
#     """
#     WebRTC Video Track that pulls frames from the threaded SIYI Camera instance.
#     """
...
#         if quality_profile == self.current_quality:
#             return
#         print(f"⚠️ Quality change to {quality_profile} requested (Pipeline update required in SIYICam)")
#         self.current_quality = quality_profile
...
#             # 1. Fetch latest frame from thread (non-blocking)
#             frame_bgr = self.siyi_cam.get_frame()

#             # 2. Handle missing frames (warmup or camera disconnected)
#             if frame_bgr is None:
#                 self.use_dummy = True
#                 # Create a black "Waiting" frame
#                 frame = np.zeros((480, 640, 3), dtype=np.uint8)
#                 cv2.putText(frame, "Waiting for SIYI Stream...", (100, 240),
#                             cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
#             else:
#                 self.use_dummy = False
#                 # Convert BGR (OpenCV) -> RGB (WebRTC)
#                 frame = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2RGB)

#             # 3. Object Detection (Background Task)
#             self.frame_counter += 1
#             if self.detection_callback and not self.use_dummy:
#                 if (self.frame_counter % self.detection_skip_frames == 0):
#                     # Pass the original BGR frame to detection
#                     asyncio.create_task(self.detection_callback(frame_bgr))

#             # 4. Wrap in VideoFrame
#             video_frame = VideoFrame.from_ndarray(frame, format="rgb24")
#             video_frame.pts = pts
#             video_frame.time_base = time_base
...
#             if self.video_health_monitor:
#                 self.video_health_monitor.on_frame_send_failed(str(e))
#             # Return dummy frame on crash to keep stream alive
#             frame = np.zeros((480, 640, 3), dtype=np.uint8)
#             video_frame = VideoFrame.from_ndarray(frame, format="rgb24")
#             pts, time_base = await self.next_timestamp()
#             video_frame.pts = pts
...
#         try:
#             # Note: We rely on defaults in siyi_cam2.py. 
#             # If you need specific setup calls (follow mode etc), uncomment below:
#             # self.cam.setup_cam() 
#             self.cam.start()
#         except Exception as e:
...
#             )
#             print("🔍 Surveillance Mode: Detection ENABLED")
#         else:
#             self.detector = None
#             print("📦 Payload Mode: Detection DISABLED")

#         # Rescue Mission Specifics
#         if self.drone_type == "rescue":
#             self.fire_queue = []
#             self.current_mission = None
...
#                 position_update_callback=self.update_dummy_position
#             )
#             print("🔥 Fire Extinguisher Logic ENABLED")
...
#     async def authenticate(self):
#         print(f"🔐 Authenticating as {DRONE_ID}...")
#         async with aiohttp.ClientSession() as session:
#             try:
#                 async with session.post(f"{BACKEND_URL}/api/v1/auth/drone", json={
#                     "drone_id": DRONE_ID,
...
#     async def connect_mavsdk(self):
#         if USE_DUMMY_TELEMETRY:
#             print("⏭️  Skipping MAVSDK connection (Dummy Mode)")
#             return

#         print("🔌 Connecting to MAVSDK...")
#         # Adjust connection string as needed (udp:// or serial://)
...
#         # 2. Create new Video Track with threaded camera instance
#         self.video_track = WebcamVideoTrack(
...
#         radius = 0.0001
#         angle = (self.dummy_time * 0.1) % (2 * math.pi)
#         if not self.is_airborne and self.drone_type == "rescue":
#              # Static if on ground
#              pass
#         else:
#             self.dummy_lat = DUMMY_START_LAT + radius * math.cos(angle)
...
#             "gps": {"lat": self.dummy_lat, "lon": self.dummy_lng, "alt_rel_m": self.dummy_alt},
#             "orientation": {"roll": 0, "pitch": 0, "yaw": self.dummy_heading},
#             "status": {"flight_mode": "POSCTL", "is_armed": True},
...
#             try:
#                 if USE_DUMMY_TELEMETRY:
#                     data = self._generate_dummy_telemetry()
#                     telemetry = {
...
#             await asyncio.sleep(1.0) # 1Hz Telemetry

#     # --- Telemetry Readers (MAVSDK) ---
#     async def _position_reader(self):
#         async for pos in self.drone.telemetry.position():
#             self.latest.setdefault("gps", {})
#             self.latest["gps"].update({
...
#             # Handle enum naming variations depending on MAVSDK version
#             ft = getattr(info, "fix_type", 0)
#             val = 0
#             if str(ft) in fix_map: val = fix_map[str(ft)] # if enum
#             elif ft in fix_map.values(): val = ft # if int
#             else: val = 3 # assume 3d fix if unsure/complex enum
            
#             self.latest["gps_info"] = {"num_satellites": info.num_satellites, "fix_type": val}

#     # --- Fire Detection ---
#     async def process_frame_for_detection(self, frame_bgr):
#         """Analyze frame for fire markers"""
#         if self.detector is None: return

#         try:
#             # Use current telemetry for geo-tagging
#             gps = self.latest.get("gps", {})
...
#                 }
#                 await self.websocket.send(json.dumps(msg))
#                 print(f"🔥 FIRE DETECTED at {lat}, {lon}")
#         except Exception as e:
...
#             await self.websocket.send(json.dumps({"type": "response", "cmd": cmd, "status": "ok"}))
#         except Exception as e:
#             print(f"Command failed: {e}")
#             await self.websocket.send(json.dumps({"type": "response", "cmd": cmd, "status": "error", "message": str(e)}))
...
#         try:
#             self.cam.move(yaw_deg=yaw, pitch_deg=pitch)
#             self.cam.zoom(zoom_level=zoom)
#             print(f"🎥 Gimbal: Pitch {pitch}, Yaw {yaw}, Zoom {zoom}")
#         except Exception as e:
#             print(f"Gimbal error: {e}")

#     # --- Extinguish Mission (Rescue Drone) ---
#     async def handle_extinguish_fire(self, data):
#         if self.drone_type != "rescue":
#             print("⚠️ Ignoring extinguish command (Not a rescue drone)")
#             return

#         locations = data.get("fire_locations", [])
#         if not locations: return

#         print(f"🔥 Received {len(locations)} fire targets")
...
#             # Arm and Takeoff if needed
#             if not self.is_airborne and not USE_DUMMY_TELEMETRY:
#                 try:
#                     print("🚁 Auto-Takeoff for Mission...")
#                     await self.drone.action.arm()
#                     await self.drone.action.set_takeoff_altitude(15.0)
#                     await self.drone.action.takeoff()
...
#             fire_list = self.fire_queue.copy()
#             gps_list = [(f["target_lat"], f["target_lon"], 15.0) for f in fire_list]

#             try:
#                 # CRITICAL: Pass self.cam (SIYICam instance) to logic
#                 # go_align_drop must be compatible with calling cam.get_frame()
#                 await go_align_drop(self.drone, self.cam, gps_list)
...
#                         if t in ["webrtc_answer", "ice_candidate"]:
#                             await self.handle_signaling(data)
#                         elif t == "request_offer":
#                             await self.send_webrtc_offer()
#                         elif t == "command":
#                             await self.handle_command(data)
#                         elif t == "gimbal_control":
#                             await self.handle_gimbal_control(data)
#                         elif t == "extinguish_fire":
#                             await self.handle_extinguish_fire(data)
#                     except Exception as e:
#                         print(f"Msg error: {e}")
...
#         if self.cam:
#             try:
#                 self.cam.stop() # Kill camera thread
#             except: pass
#         if self.pc:
#             await self.pc.close()
#         if self.websocket:
...
class TelemetryHealthMonitor:
    ...
    def on_telemetry_send_failed(self, error=None):
        ...
        self.total_failures += 1
        if self.consecutive_send_failures >= self.max_consecutive_failures:
            print(f"\n❌ TELEMETRY TRANSMISSION FAILURE: Connection lost")
            sys.exit(EXIT_BACKEND_UNAVAILABLE)
